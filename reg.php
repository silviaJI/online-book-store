<?php
	...
	//注册信息判断
  	if (!preg_match("/[a-za-z0-9]+@[a-za-z0-9]+.com/",$email))  
  		{  
	...
	include('conn.php');
	//检测用户名是否已经存在
	$check_query = mysql_query("select Id from user_info where Id='$email' limit 1");
	if(mysql_fetch_array($check_query)){
    echo '错误：用户名 ',$email,' 已存在。<a href="javascript:history.back(-1);">返回</a>';
    exit;
	}
	/*$check_query = "select Id from user_info where Id='$email' limit 1";
	$stmt = $connection->query($check_query);
	$stmt = $db->prepare($sql,array(PDO::FETCH_ASSOC));
	$stmt ->execute();
	if(mysql_fetch_array($stmt)){
	    echo '错误：邮箱 ',$email,' 已被注册。<a href="javascript:history.back(-1);">返回</a>';
	    exit;
	}*/
	//写入数据
	$password = MD5($psd);
	$sql = "INSERT INTO user_info(Id,psd,tel,bir)VALUES('$email','$password','$tel',
	$bir)";
	if(mysql_query($sql, $conn)){
	    exit('用户注册成功！点击此处 <a href="login.html">登錄</a>');
	} else {
